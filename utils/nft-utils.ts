...
export async function mintPiggyIDNFT(metadataURI: string, walletAddress: string): Promise<MintResult> {
  try {
    ...
    return {
      success: false,
      error:
        "This function needs to be implemented with wagmi hooks. Call mintPiggyID on contract 0x7FA5212be2b53A0bF3cA6b06664232695625f108 with arguments: [walletAddress, ipfsHash]",
    }
  } catch (error: any) {
    return {
      success: false,
