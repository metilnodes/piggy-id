...
export async function POST(request: NextRequest) {
  try {
    ...
    const discord_username = body?.discord_username ? String(body.discord_username).trim() : null

    // optional fields (for gas funding)
    ...
    const { searchParams } = new URL(request.url)
    const createStub = searchParams.get("create_stub") === "true"
    const force = searchParams.get("force") === "true"

    // Check if user exists
    const current = await sql`
      SELECT tips_wallet_address, tips_gas_funded_at, tips_gas_funding_tx, wallet_address, token_id
      FROM user_identities
    ...
    if (!current[0]) {
      ...
      // create_stub=true: create minimal record
      if (!tips_wallet_address) {
        return NextResponse.json({ error: "tips_wallet_address is required when creating stub" }, { status: 400 })
      }

      // Insert stub record with minimal fields
      await sql`
        ...
        VALUES (
          ${discord_id},
          ${discord_username},
          ${tips_wallet_address.toLowerCase()},
          NULL,
          NULL,
      ...
      return NextResponse.json({
        ...
        tips_gas_funded_at: tips_gas_funded_at || null,
        tips_gas_funding_tx: tips_gas_funding_tx || null,
      })
    }
    ...
    const newTipsWallet = tips_wallet_address ? tips_wallet_address.toLowerCase() : current[0].tips_wallet_address

    const newGasFundedAt = tips_gas_funded_at !== undefined ? tips_gas_funded_at : current[0].tips_gas_funded_at

    const newGasFundingTx = tips_gas_funding_tx !== undefined ? tips_gas_funding_tx : current[0].tips_gas_funding_tx

    const newDiscordUsername = discord_username !== null ? discord_username : undefined

    // no-op guard
    if (!newTipsWallet && newGasFundedAt === undefined && newGasFundingTx === undefined && !newDiscordUsername) {
      return NextResponse.json({ error: "No fields to update" }, { status: 400 })
    }

    if (newDiscordUsername !== undefined) {
      await sql`
        ...
        SET
          ...
          discord_username = ${newDiscordUsername}
        WHERE discord_id = ${discord_id}
      `
    } else {
      await sql`
    ...
    const updated = await sql`
      SELECT tips_wallet_address, tips_gas_funded_at, tips_gas_funding_tx, discord_username
      FROM user_identities
      WHERE discord_id = ${discord_id}
      LIMIT 1
    `
    ...
    return NextResponse.json({
      ok: true,
      discord_id,
      discord_username: updated[0]?.discord_username || null,
