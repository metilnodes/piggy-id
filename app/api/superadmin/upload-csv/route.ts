...
export async function POST(request: NextRequest) {
  try {
    ...
    // Clear existing superpoker codes and assignments
    ...
    return NextResponse.json({
      success: true,
      count: codes.length,
      message: `Successfully uploaded ${codes.length} superpoker codes`,
    })
  } catch (error) {
    console.error("Failed to upload superpoker CSV:", error)
