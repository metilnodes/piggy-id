...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Email verification request:", { email, walletAddress })
    ...
    const sameWalletEmail = await sql`
      SELECT email FROM user_identities 
      WHERE email = ${emailLc} AND wallet_address = ${walletLc}
    `

    if (sameWalletEmail.length > 0) {
      console.log("[v0] Email already connected to this wallet")
      return NextResponse.json({ success: true, message: "Email already connected to your account" })
    }
    ...
    console.log("[v0] Generated verification token for:", emailLc)
    ...
    const emailResult = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <a href="${verificationUrl}" style="display: inline-block; background-color: #007cba; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; margin: 16px 0;">Verify Email</a>
